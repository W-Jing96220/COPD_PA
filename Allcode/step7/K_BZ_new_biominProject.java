...
public class K_BZ_new_biominProject {
	...
	//for a vertex v, deleted_step1(v) returns true if sum(pi) < threshold_sumP
	...
	public K_BZ_new_biominProject(String basename, double eta, int L, int precision, double threshold_sumP,
			int threhsold_etaDegree, String DPtype) throws Exception {
...
//		processors = 7;
		...
		remove_byFilter_sumP_sum_Parallel();  //removing vertices based on sumP
		//remove_byFilter_sumP_sum_Sequential(); //the same function, but run sequentially
		...
		System.out.println("Total number of alive vertices are:" + num_alive);
		
		//now store them in an array
		alive_vertices = IntStream.range(0, deg.length).filter(i -> (deg[i] >=threhsold_etaDegree)).toArray();
	...
	public int compute_etaDegree_foraliveVertices_filter_Parallel() {
		...
		System.out.println("Number of vertices alive after filtering by eta-degree=" + cnt_alive + "==" + (n-deleted_step2.cardinality()));
		...
		for (int u = 0; u < n; u++) {
			if (deleted_step1.get(u) == false) { //if u is still alive
				...
				if (deg[u] >= threhsold_etaDegree) {
					...
					deleted_step2.set(u); // this u is deleted in step 2
				}
	...
	public void remove_byFilter_sumP_sum_Parallel() {
		...
		BigInteger cnt_alive = IntStream.range(0, n).parallel().map(u -> {
			...
			//sumP_sum[0]=sumP for a vertex u
		...
		System.out.println("Number of vertices alive after filtering by sumP_sum=" + cnt_alive + "==" + (n-deleted_step1.cardinality()));

	}

	public void remove_byFilter_sumP_sum_Sequential() {
...
//        args = new String[] { "Biomine_data_unique_renamed-proc.w", "0.5", "1000", "17", "28", "46"};
		//DBLP
//		args = new String[] { "DBLP-proc.w", "0.5", "1000", "16", "5", "10"};

		if (args.length < 4) {
			System.err.println("Specify: basename eta L precision thresh_sumP threhsold_etaDegree DPtype(optional)");
			System.exit(1);
		}

		System.out.println("Basename: " + args[0] + " eta: " + args[1] + " L: " + args[2] +" precision: " + args[3] + " stage-1-threshold: " + args[4] + " stage-2-threshold: " + args[5]);
		...
		double thresh_sumP = Double.parseDouble(args[4]);
		int threhsold_etaDegree = Integer.parseInt(args[5]);
		String DPtype = args.length == 7 ? args[6] : "DP";

		System.out.println("Starting " + basename);
		long startTime = System.currentTimeMillis();

		K_BZ_new_biominProject tt = new K_BZ_new_biominProject(basename, eta, L, precision, thresh_sumP,
				threhsold_etaDegree, DPtype);
		...
		tt.writeResults(res, basename+"eta-" + eta + "-bz-modified.txt");
